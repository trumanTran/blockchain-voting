...
def create_key_table():
    ...
    cur.execute('CREATE TABLE IF NOT EXISTS key_table (mid TEXT UNIQUE NOT NULL, ip_add TEXT UNIQUE, port TEXT UNIQUE, pub_key TEXT UNIQUE NOT NULL)')
    #Table created.

def insert_to_key_table(mid, ip_add, port, pub_key):
    #Insert to the key table the values above.
    cur.execute('INSERT INTO key_table VALUES (?,?,?,?)', (mid, ip_add, port, pub_key))
    con.commit()
