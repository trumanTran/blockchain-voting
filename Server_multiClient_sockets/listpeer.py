...
returns false'''
#----------------------------------------------------------------------------------------------------------------------#
def verify_incoming_peer(machineID, key, ip_address, port_number):
...
def handle_incoming_peer(connection):
    ...
    inc_msg = pickle.loads(incoming_message)

    machine_id = inc_msg[0]
    key = inc_msg[1]
    ip_address = inc_msg[2]
    port_number = inc_msg[3]
    command = inc_msg[4]
    message = inc_msg[5]
    ...
    if verify_incoming_peer(connection, machine_id, key, ip_address, port_number):
        ...
        connection.send(pickle.dumps(outgoing_message))
# ---------------------------------------------------------------------------------------------------------------------#
#----------------------------------------------------------------------------------------------------------------------#
...
def incoming_command_handler(connection, ip_address, port_number, command, incoming_message):
    ...
    if command == "PEER":
        ...
        for i in incoming_message:
            #Ok now we take each tuple and have a real giggle with it.
            #Set the info.
            #Machine_ID, Key, IP Address, and Port in that order.
            peers.append(Peer_Info(incoming_message[i][0], incoming_message[i][1], incoming_message[i][2], incoming_message[i][3]))
        '''
        index = 0
    ...
    # -------------------  Peer receives command to send copy of registered peer list ---------------------------------#
        ...
        for x in registered_peers:
            #Take a tuple of all relevant info, and append it to the list.
            list_peer += (str(x.machineID), str(x.privateKey), str(x.ipAddress), str(x.portNumber))
        #Great, we got everything. Now append the entire list to the message as the actual message object.
        outgoing_message += list_peer
    ...
    #----------------------------- Peer receives command to update it's blockchain ---------------------------------#
        ...
        connection.send(pickle.dumps(outgoing_message))
    #---------------------------------------------------------------------------------------------------------------#
    #-------- Peer receives confirmation that other peer has received new block and added it to blockchain ---------#
